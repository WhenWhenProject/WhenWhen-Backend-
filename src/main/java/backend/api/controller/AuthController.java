...
import backend.api.service.UserRefreshTokenService;
...
public class AuthController {
    ...
    public ApiResponse<String> refreshToken(HttpServletRequest request, HttpServletResponse response) {
        // access token 확인
        String accessToken = HeaderUtil.getJwtToken(request);
        JwtToken jwtToken = jwtTokenProvider.convertToJwtToken(accessToken);
        ...
        String refreshToken = CookieUtil.getCookie(request, REFRESH_TOKEN)
                .map(Cookie::getValue)
                .orElse((null));
        JwtToken authRefreshToken = jwtTokenProvider.convertToJwtToken(refreshToken);
        ...
        UserRefreshTokenDto userRefreshTokenDto = userRefreshTokenService.findByUsernameAndRefreshToken(username, refreshToken);
        ...
        if (validTime <= THREE_DAYS_MSEC) {
            ...
            // DB에 refresh 토큰 업데이트
            userRefreshTokenService.changeRefreshToken(userRefreshTokenDto.getRefreshToken(), authRefreshToken.getToken());
